...
train_df = all_df.sample(frac=0.7, random_state=422).reset_index()
test_df = all_df.drop(train_df.index).reset_index()
...
np.mean([pred == target for (pred, target) in zip(predicted, test_categories)])
...
# perfect accuracy??? ===========================================================
np.mean([pred == target for (pred, target) in zip(predicted2, test_categories)])
